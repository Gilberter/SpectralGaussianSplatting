...
def _spherical_harmonics_hs(
    degrees_to_use: int,
    dirs: torch.Tensor,                        # [..., 3]
    coeffs: torch.Tensor,                      # [..., K, B]
    masks: Optional[torch.Tensor] = None,      # [...] bool
):
    """
    Hyperspectral SH with optional mask support (mirrors CUDA kernel behavior).
    Masked-out elements (masks=False) return 0, same as CUDA kernel early return.
    """
    num_bases = (degrees_to_use + 1) ** 2
    assert num_bases <= coeffs.shape[-2], \
        f"Not enough SH coefficients: need {num_bases}, got {coeffs.shape[-2]}"

    batch_dims = dirs.shape[:-1]
    B = coeffs.shape[-1]  # spectral bands

    dirs = F.normalize(dirs, p=2, dim=-1)

    # bases: [..., K]
    bases = torch.zeros(*batch_dims, coeffs.shape[-2], dtype=dirs.dtype, device=dirs.device)
    bases[..., :num_bases] = _eval_sh_bases_fast(num_bases, dirs)

    # [..., B]
    colors = (bases[..., None] * coeffs).sum(dim=-2)

    # Apply mask — masked elements return 0 (mirrors CUDA early return)
    if masks is not None:
        colors = colors * masks[..., None].to(colors.dtype)  # [..., 1] broadcast over B
