...
def rasterization(
    ...
    # RGB Assert if RGB Mode
        ...
        if rendering_mode == "rgb_sh":
            assert sh_degree >= 0, "Rendering mode RGB with SH, sh_degree [0,1,2,3]" 
        else:
            sh_degree = None
    ...
    if use_hyperspectral:
        ...
        if sh_hyperspectral:
            ...
            # treat colors as SH coefficients, should be in shape [..., N, K, Bands] or [..., C, N, K, Bands]
            # Allowing for activating partial SH bands
            # print("num batch",num_batch_dims)
    ...
    if distributed:
        if packed:
            print("we dont mind packed")

        else:
            print("we dont mind")
        print("we dont use distributed there is just one gpu")
    ...
    # we dont use this but could be useful
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        if with_eval3d:
            print("Whether to calculate Gaussian response in 3D world space, instead of 2D image space. Default is False")
        else:
