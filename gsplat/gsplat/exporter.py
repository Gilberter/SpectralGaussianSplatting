...
# splat 2 splat bytes hs -> This can be rendered 
# Return the binary Splat file. Supported by antimatter15 viewer.
# Implement a splat 2 splat with spherical harmonics
# Implement a splat 2 plat for the Superviewer  "Return the binary compressed Ply file. Used by Supersplat viewer


def splat2splat_bytes_hyperspectral(
    means: torch.Tensor,
    scales: torch.Tensor,
    quats: torch.Tensor,
    opacities: torch.Tensor,
    ...
    rgb = spectrum_to_rgb(spectrum)  # (N, 3)

    colors = torch.cat(
        [rgb, torch.sigmoid(opacities).unsqueeze(-1)],
        dim=1
    )
    ...
    rots = quats / torch.linalg.norm(quats, dim=1, keepdim=True)
    rots = (rots * 128 + 128).clamp(0, 255).to(torch.uint8)
...
def export_splats(
    ...
    format: Literal["ply", "splat", "ply_compressed", "ply_hs"] = "ply",
    save_to: Optional[str] = None,
) -> bytes:
    """Export a Gaussian Splats model to bytes.
    The four supported formats are:
    - ply: A standard PLY file format. Supported by most viewers. (RGB, 3 channels)
    - splat: A custom Splat file format. Supported by antimatter15 viewer. (RGB, 3 channels)
    - ply_compressed: A compressed PLY file format. Used by Supersplat viewer. (RGB, 3 channels)
    - ply_hs: A PLY file format for hyperspectral data. Uses sh0 only with N spectral bands.
    ...
    Args:
        ...
        quats (torch.Tensor): Splat quaternions. Shape (N, 4)
        opacities (torch.Tensor): Splat opacities. Shape (N,)
        sh0 (torch.Tensor): Spherical harmonics degree 0.
                            RGB formats: Shape (N, 1, 3)
                            ply_hs format: Shape (N, 1, B) where B is the number of spectral bands
        shN (torch.Tensor, optional): Higher-degree spherical harmonics. Shape (N, K, 3).
                                      Required for "ply" and "ply_compressed". Not used for "ply_hs".
        format (str): Export format. Options: "ply", "splat", "ply_compressed", "ply_hs". Default: "ply"
        save_to (str): Output file path. If provided, the bytes will be written to file.
    """
    total_splats = means.shape[0]

    # --- Core shape assertions (format-agnostic) ---
    ...
    assert sh0.ndim == 3 and sh0.shape[0] == total_splats and sh0.shape[1] == 1, \
        "sh0 must be of shape (N, 1, B) where B >= 1"

    # --- Format-specific assertions ---
    is_hyperspectral = format == "ply_hs"

    if not is_hyperspectral:
        num_bands = sh0.shape[2]
        assert num_bands == 3, \
            f"RGB formats require sh0 with 3 channels (got {num_bands}). Use 'ply_hs' for hyperspectral data."

        if format in ("ply", "ply_compressed"):
            assert shN is not None, f"shN is required for format '{format}'"
            assert (
                shN.ndim == 3 and shN.shape[0] == total_splats and shN.shape[2] == 3
            ), f"shN must be of shape (N, K, 3), got {shN.shape}"
    else:
        num_bands = sh0.shape[2]
        assert num_bands >= 1, "ply_hs format requires at least 1 spectral band in sh0"
        if shN is not None:
            import warnings
            warnings.warn("shN is not used for 'ply_hs' format and will be ignored.")

    # --- Reshape spherical harmonics ---
    sh0 = sh0.squeeze(1)  # Shape (N, 3) for RGB or (N, B) for hyperspectral

    if shN is not None and not is_hyperspectral:
        shN = shN.permute(0, 2, 1).reshape(total_splats, -1)  # Shape (N, K * 3)
    ...
    if shN is not None and not is_hyperspectral:
        invalid_mask = invalid_mask | torch.isnan(shN).any(dim=1) | torch.isinf(shN).any(dim=1)
    ...
    # --- Dispatch to format-specific serializer ---
    if format == "ply":
        ...
        data = splat2splat_bytes_hyperspectral(means, scales, quats, opacities, sh0)
    else:
